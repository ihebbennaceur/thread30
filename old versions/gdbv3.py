...
BASE_DIR = os.path.join(os.getcwd(), "gp3")
IMAGE_EXTENSIONS_REGEX = re.compile(r"(?i)(\.jpg|\.jpeg|\.png|\.gif)")

MAX_PRODUCTS_PER_ITER = 10
...
def download_images(image_urls: List[str], product_id: str, category: str) -> Tuple[int, List[Tuple[str, str]], List[int], List]:
    ...
    for idx, img_url in enumerate(image_urls):
        ...
        arequests.append([full_url, 2])  
        airq2iimg.append(idx)

    bj = curl_batch_job()
    bj.set_jobs(arequests, 10, None) #j'ai change to 10

    while True:
        if bj.periodic():
            break
        sleep_mcs(50_00) #j'ai changé to 5 ms
    ...
    for i in range(len(bj.arequests)):
        ...
        if rq.get_resp_code() == 200:
            ...
            hiimg_err_final.append((rq.get_resp_code(), "Unknown error"))
